...
from openstack_dashboard.settings import BILLING_BASE_URL
...
class UsageView(tables.DataTableView):
    ...
    def get_data(self):
        try:
            ...
            self.billingitems = self.get_billingitems()
    ...
    def get_billingitems(self, search_opts=None):
        try:
            account_id=self.request.session['account']['account_id']
            billingitems = api.billing.request('/account/detail/%s' % account_id)
        except Exception:
            billingitems = []
            exceptions.handle(self.request,
                              _('Unable to retrieve billing list.'))
        billing = billingitems['account']
        bumber = float(billing['credit_line'])+float(billing['cash_balance'])+float(billing['gift_balance'])
        billing['sumbalance']= float(bumber)     
        billing['base_url']= BILLING_BASE_URL
        return billing
    ...
    def get_context_data(self, **kwargs):
        ...
        context['billing'] = self.billingitems
        context['charts'] = []
