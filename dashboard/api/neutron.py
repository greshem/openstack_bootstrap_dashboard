...
from dashboard.api import network_base
...
class SecurityGroup(NeutronAPIDictWrapper):
    ...
    def __init__(self, sg, sg_dict=None,rules_total=0):
        if sg_dict is None:
            sg_dict = {sg['id']: sg['name']}
        sg['rules'] = [SecurityGroupRule(rule, sg_dict)
                       for rule in sg['security_group_rules']]
        sg['rules_total']=rules_total
...
class SecurityGroupManager(network_base.SecurityGroupManager):
    ...
    def _list(self, **filters):
        result = self.client.list_security_groups(**filters)
        if isinstance(result, tuple):
            secgroups,total = result
            secgroups = secgroups.get('security_groups')
        else:
            secgroups = result.get('security_groups')
            total = len(secgroups)
        return [SecurityGroup(sg) for sg in secgroups],total

    def list(self,**filters):
        tenant_id = self.request.user.tenant_id
        return self._list(tenant_id=tenant_id,**filters)
    ...
    def _sg_name_dict(self, sg_id, rules):
        ...
        result = self.client.list_security_groups(id=related_ids,
                                                       fields=['id', 'name'])
        if isinstance(result, tuple):
            related_sgs = result[0]
        else:
            related_sgs = result
        related_sgs = related_sgs.get('security_groups')
        return dict((sg['id'], sg['name']) for sg in related_sgs)

    def get(self, sg_id,**param):
        secgroup = self.client.show_security_group(sg_id,**param).get('security_group')
        sg_dict = self._sg_name_dict(sg_id, secgroup['security_group_rules'])
        return SecurityGroup(secgroup, sg_dict,rules_total=secgroup['security_group_rules_total'])
    ...
    def list_by_instance(self, instance_id):
        ...
        ports = port_list(self.request, device_id=instance_id)[0]
        sg_ids = []
        for p in ports:
...
class FloatingIpManager(network_base.FloatingIpManager):
    ...
    def list_targets(self):
        tenant_id = self.request.user.tenant_id
        ports = port_list(self.request, tenant_id=tenant_id)[0]
    ...
    def _target_ports_by_instance(self, instance_id):
        ...
        return port_list(self.request, **search_opts)[0]
...
def network_list(request, **params):
    ...
    for n in networks:
        ...
        n['subnets'] = [subnet_dict[s] for s in n.get('subnets', []) if
                        s in subnet_dict]
    return [Network(n) for n in networks], total
...
def network_list_for_tenant(request, tenant_id, include_external=False,
    ...
    networks = network_list(request, tenant_id=tenant_id, **params)
...
def port_list(request, **params):
    ...
    result=neutronclient(request).list_ports(**params)
    if isinstance(result,tuple):
        ports,total=result
        ports = ports.get('ports')
        return [Port(p) for p in ports], total
    ports = result.get('ports')
    return [Port(p) for p in ports], len(ports)
...
def router_list(request, **params):
    result=neutronclient(request).list_routers(**params)
    if isinstance(result,tuple):
        routers,total=result
    else:
        routers = result
    routers = routers.get('routers')
    total = len(routers)
    return [Router(r) for r in routers], total
