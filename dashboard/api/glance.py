...
def image_list_detailed(request, marker=0, sort_dir='desc',
                        sort_key='created_at', filters=None, paginate=False,
                        reversed_order=False,limit=10):
    ...
    images_iter = glanceclient(request).images.list(**kwargs)
    if paginate:
        images,totals = zip(*list(itertools.islice(images_iter, int(limit))))
