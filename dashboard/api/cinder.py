...
def volume_list_paged(request, search_opts=None,limit=None, marker=None, paginate=True,
                      sort_dir="desc"):
...
#        page_size = utils.get_page_size(request)
        ...
        volumes_data,total=c_client.volumes.list(search_opts=search_opts,
                                       limit=limit,
                                       marker=marker,
                                       sort=sort)
...
#            volumes, page_size, marker, sort_dir)
        ...
        volumes_data=c_client.volumes.list(search_opts=search_opts)
        for v in volumes_data:
            v.transfer = transfers.get(v.id)
            volumes.append(Volume(v))
